...
const BasicInfo = ({state, error, handleChange}) => {
    return(
        ...
        {error.healthScore && <p className="errorMessage errorMessage">{error.healthScore}</p>}
        <input type="number" placeholder="Time: (min: 1 - max: 100) minutes" name="time" value={state.time} onChange={handleChange} />
        {error.time && <p className="errorMessage errorMessage">{error.time}</p>}
        ...
        {error.summary && <p className="errorMessage errorMessage">{error.summary}</p>}
      </div>
    );
};
